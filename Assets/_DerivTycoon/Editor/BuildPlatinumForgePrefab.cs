...
namespace DerivTycoon.Editor
{
    public static class BuildPlatinumForgePrefab
    {
        ...
        {
            // ????????? Colors ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
            var cConcrete   = new Color(0.55f, 0.54f, 0.52f);   // concrete walls
            var cDarkMetal  = new Color(0.28f, 0.27f, 0.26f);   // dark steel
            var cMetal      = new Color(0.52f, 0.54f, 0.58f);   // steel/platinum
            var cPlatinum   = new Color(0.88f, 0.92f, 0.96f);   // platinum shine
            var cBrick      = new Color(0.48f, 0.38f, 0.30f);   // fire-brick
            var cEmber      = new Color(1.00f, 0.40f, 0.05f);   // hot ember glow
            var cHeat       = new Color(1.00f, 0.70f, 0.10f);   // yellow heat
            var cSoot       = new Color(0.18f, 0.16f, 0.14f);   // soot-stained
            var cPipe       = new Color(0.42f, 0.44f, 0.46f);   // grey pipe
            var cWarning    = new Color(0.90f, 0.65f, 0.05f);   // hazard yellow
            ...
            // ????????? Main Forge Building (squat, heavy industrial) ????????????????????????????????????
            ...
            // ????????? Furnace Bay (front, with fire-brick surround) ????????????????????????????????????
            // Fire-brick arch surround
            ...
            // Right chimney (slightly shorter)
            ...
            // ????????? Platinum Ingot Output Conveyor ?????????????????????????????????????????????????????????????????????????????????
            // Conveyor belt (low box)
            ...
            Debug.Log($"[BuildPrefab] PlatinumForgePrefab saved ??? {path} ({root.transform.childCount} children)");
        }
    }
}
